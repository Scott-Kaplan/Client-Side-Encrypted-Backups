...
* Here are some examples if you want to change the timestamp of the timeStampMarker file -
	* 	This will alter the default behavior of what files are considered new or changed that will go into the backup.  Instead a backup will now include all files that have changed or are new post the timestamp on the timeStampMarker file.
