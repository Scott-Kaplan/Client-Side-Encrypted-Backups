...
## Overview
There are a number of encrypted backup programs available, however most can't prevent creating corrupt backups.  Client-Side-Encrypted-Backups always produces reliable backups.  Since corruption is not possible with Client-Side-Encrypted-Backups, only one full backup is needed (the first time that you do a backup).
## Requirements
...
## Notes
* If you don't want to be prompted for you SFTP passord every time you do a backup (or a restore),
...
* [ccrypt](http://ccrypt.sourceforge.net/) provides the encryption.
